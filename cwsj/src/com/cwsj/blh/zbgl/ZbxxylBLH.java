...
public class ZbxxylBLH extends BaseBLH{
 ...
 */
	public BaseResponseEvent initQyth(BaseRequestEvent req) throws Exception{
		...
		List<Map> sjxxlList =(List<Map>) BaseDAO.getInstance().findForList("YsjglMapper.listQuerrySjxxl", null);
		res.addSelect("sjxxlSelect", sjxxlList);
		List<Map> listysj =(List<Map>) BaseDAO.getInstance().findForList("SjsjtsMapper.listQuerryqyTh", USERID);
		res.addGrid("listysj", listysj);		
 ...
 */
	public BaseResponseEvent initZbsjsh(BaseRequestEvent req) throws Exception{
		...
		List<Map> sbblist =(List<Map>) BaseDAO.getInstance().findForList("SjsjtsMapper.listQuerrysbb", LSH);
 ...
 */
	public BaseResponseEvent initSjcyZbsjsh(BaseRequestEvent req) throws Exception{
		...
		dgmap.put("dglxselect", "02");
		dgmap.put("userId", USERID);
		List<Map> listywdg =(List<Map>) BaseDAO.getInstance().findForList("SjsjtsMapper.listQuerrygzdgsjcy", dgmap);		
		res.addGrid("listywdg", listywdg);	
		dgmap=new HashMap();
		dgmap.put("LSH", LSH);
		dgmap.put("dglxselect", "03");
		dgmap.put("userId", USERID);
		List<Map> listywjgdg =(List<Map>) BaseDAO.getInstance().findForList("SjsjtsMapper.listQuerrygzdgsjcy", dgmap);		
		res.addGrid("listywjgdg", listywjgdg);			
		res.addObject("LCSLH", LCSLH);
		res.addObject("LSH", LSH);
	...
	public BaseResponseEvent qyyjth(BaseRequestEvent req) throws Exception{
		...
		String LSH=(String) req.getAttr("LSH");
		PersistenceDAO persistenceDAO=this.getPersistenceDAO();
		BaseDAO.getInstance().save("SjsjtsMapper.thSjhzb", LSH);
		return res;
	}
}
