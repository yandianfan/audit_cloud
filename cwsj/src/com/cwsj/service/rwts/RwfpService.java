...
public class RwfpService {
	...
	public BaseResponseEvent rwfpinit(BaseRequestEvent req) throws Exception{
		...
		List<Map> fpList =(List<Map>) dao.findForList("GzlXMapper.listQuerryRwfp", param);
	...
	public BaseResponseEvent querry(BaseRequestEvent req) throws Exception{
		...
		List<Map> fpList =(List<Map>) dao.findForList("GzlXMapper.listQuerryRwfp", cxtjform);
		res.addGrid("fpList", fpList);
		return res;
	}	
	...
	public BaseResponseEvent rwfp(BaseRequestEvent req) throws Exception{
		...
		dao.update("GzlXMapper.updateRwfp", param);
		dao.save("YsjglMapper.insertBzsjx", param);
        res.addMessage("分配成功");
		return res;
	}
	...
	public BaseResponseEvent selectTsry(BaseRequestEvent req) throws Exception{
		...
		List<Map> gzllclist =(List<Map>) dao.findForList("GzlXMapper.listQuerryYwhj", params);
	...
	public BaseResponseEvent rwrgfp(BaseRequestEvent req) throws Exception{
		...
		if(slmap!=null&&YWHJ_DM!=null){		  
		  if(lasthj.equals(YWHJ_DM)){
			  ...
			  dao.save("GzlXMapper.insertEndLzxx", param);
		  }else{		
			  YWHJ_DM=String.valueOf(Integer.parseInt(YWHJ_DM)+1);
			  param.put("YWHJ_DM", YWHJ_DM);
			  dao.save("GzlXMapper.sendLcslxx", param);	
			  ...
			  dao.save("GzlXMapper.insertEndLzxx", param);
			  dao.save("GzlXMapper.insertLzxx", param);
		  }		
		}else{
			dao.save("GzlXMapper.insertLcslxx", param);
			dao.save("GzlXMapper.insertLzxx2", param);
		}
        res.addMessage("分配成功");
		return res;
	}	
