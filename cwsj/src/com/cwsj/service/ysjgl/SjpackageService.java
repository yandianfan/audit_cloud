...
@Service("sjpackageService")
public class SjpackageService {
	...
	public BaseResponseEvent initPackagenr(BaseRequestEvent req) throws Exception{
		...
		List<Map> listsjb =(List<Map>) dao.findForList("YsjglMapper.listQuerrysjpackage", DJXH);
		if(LSH==null||"".equals(LSH)){
			Map mapqyxx =(Map) dao.findForObject("CommMapper.mapQuerryQyjbxx", DJXH);
 ...
 */
	public BaseResponseEvent saveSjb(BaseRequestEvent req) throws Exception{
		...
		dao.delete("YsjglMapper.deleteSjb", DJXH);
		dao.save("YsjglMapper.insertSjb", PlatformUtil.addGridcolvalue(listsjb, "DJXH", DJXH));
        res.addMessage("保存成功");
		return res;
	}		
}
