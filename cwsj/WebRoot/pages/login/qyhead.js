...
var fmid = "fhindex";
var mid = "fhindex";
function siMenu(id,fid,MENU_NAME,target,MENU_URL){
	if(id != mid){
		$("#"+mid).removeClass();
		mid = id;
	}
...
$(function(){
	$.ajax({
		type: "POST",
		url: locat+'/indexhead/getUname.do?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
		...
		success: function(data){
			//alert(data.list.length);
			 $.each(data.list, function(i, list){
				 ...
				 //登陆者资料
				 $("#user_info").html('<small>Welcome</small> '+list.XM+'');
				 
				 
				 USER_ID = list.USER_ID;//用户ID
				 hf(list.SKIN);//皮肤
				 if(list.USER_NAME != 'admin'){
...
function editPassword(){
	 ...
	 diag.Drag=true;
	 diag.Title ="修改密码";
	 diag.URL = locat+'/modifyPassword.do?USER_ID='+USER_ID;
	 diag.Width = 325;
	 diag.Height = 300;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
