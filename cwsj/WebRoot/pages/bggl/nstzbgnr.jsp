  ...
  <div class="row-fluid">
		<div class="widget-box">
			<div class="widget-header widget-header-blue widget-header-flat wi1dget-header-large">
				<h4 class="lighter">${BGOBJ.BGLX_MC}</h4>
			</div>
			<div class="widget-body">			 
			 <div class="widget-main">
				...
				<input type="hidden" name="BGLX_DM" id="BGLX_DM" value="${BGOBJ.BGLX_DM}"/>
				<input type="hidden" name="BG_XH" id="BG_XH" value="${BGOBJ.BG_XH}"/>
				<input type="hidden" name="IFHAPPEN" id="IFHAPPEN" value="${BGOBJ.IFHAPPEN}"/>
			
				<table style="width:100%;" id="xtable">
					<tr>
						<td id="bgnr" colspan="3">
							 <script id="editor" type="text/plain" style="width:96%;height:299px;">${BGOBJ.BGNR}</script>
						</td>
					</tr>
				</table>
				</div>	
...
function save(){
	...
	var BGNR = $("#CONTENT").val();
	var BGNR_TEXT = getContentTxt();
	var BG_XH=$("#BG_XH").val();
	var BGLX_DM=$("#BGLX_DM").val();
	var LSH=$("#LSH").val();
	var IFHAPPEN=$("#IFHAPPEN").val();
	
	$.ajax({
		type: "POST",
		url: getRootPath()+'/nstzbgCtrl/saveBgnr?tm='+new Date().getTime(),
    	data: {LSH:LSH,BG_XH:BG_XH,BGLX_DM:BGLX_DM,BGNR:BGNR,BGNR_TEXT:BGNR_TEXT,IFHAPPEN:IFHAPPEN},
		dataType:'json',
...
function onDefalut(){
	...
	var BGLX_DM=$("#BGLX_DM").val();
	var LSH=$("#LSH").val();
    window.location.href=getRootPath()+"/nstzbgCtrl/onDefalut?BGLX_DM="+BGLX_DM+"&LSH="+LSH;
